...
@section('title', '| Army Punishments')

@section('content')
    <div class="col-lg-10 offset-lg-1">
        <h3><i class="fa fa-user" style="font-size: large"></i> Army Punishments Awarded</h3>
        <hr>
        @include('armies.details')


        @if(request()->has('redirect'))
            {{ Form::open(['route' => ['punishments.store',$army->id,'redirect' => request()->get('redirect')], 'novalidate']) }}
        @else
            {{ Form::open(['route' => 'punishments.store', 'novalidate']) }}
        @endif
        ...
        <div class="d-flex justify-content-end">
            {{ Form::submit('Add', ['class' => 'btn btn-primary px-5']) }}
        </div>
        {{ Form::close() }}
